...
def test_deinterleave():
    ...
    deinterleaved = handler.deinterleave(test_data, 2)
    assert bytearray(deinterleaved[0]).hex() == 'f1ff0001'
    assert bytearray(deinterleaved[1]).hex() == '0000ffff'

    deinterleaved = handler.deinterleave(test_data, 4)
...
def test_interleave():
    ...
    interleaved = handler.interleave(test_data[0], test_data[1], 2)
    assert bytearray(interleaved).hex() == 'f1ff00000001ffff'

    interleaved = handler.interleave(test_data[0], test_data[1], 4)
